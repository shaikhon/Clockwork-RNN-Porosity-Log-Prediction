...
class ClockworkRNN(object):
    ...
    def _build_model(self):
        ...
        initializer_weights = tf.contrib.layers.variance_scaling_initializer(uniform=False)
        # initializer_weights = tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32)
        ...
        # activation_hidden = tf.nn.leaky_relu
        ...
        with tf.variable_scope("output"):
            ...
            self.output_W = tf.get_variable("W", shape=[self.config.num_hidden, 1], initializer=initializer_weights)  # W_O
            self.output_b = tf.get_variable("b", shape=[1], initializer=initializer_bias)                             # b_O
        ...
        with tf.variable_scope("clockwork_cell") as scope:
            ...
            for time_step in range(1, self.config.num_steps + 1):
                ...
                # update only eligible group indices
                temp = []
                ...
                # evaluate predictions point by point:
                temp2 = tf.matmul(self.state, self.output_W)
                temp2 = tf.nn.bias_add(temp2, self.output_b)
                # temp2 = activation_output(temp2)
                pred.append(temp2)

            self.predictions = tf.concat(axis=1, values=pred, name="predictions")
            ...
            # self.loss = tf.losses.mean_pairwise_squared_error(self.targets, self.predictions, weights=1.0, scope=None)
